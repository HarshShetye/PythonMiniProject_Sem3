...
Original file is located at
    https://colab.research.google.com/drive/1nm2xAz_Hip8q3nwQ0cs9vjUh9Mh8Y25V
"""

!git clone https://github.com/Vineetttt/IPCV.git

!git clone https://github.com/HarshShetye/Python_Mini_Project.git
...
added_image = add_images(image, image_new)
...
subtracted_image = subtract(image, image_new)
...
multiplied = multiply(image, image_new)
...
divided =  divide(image, image_new)
plt.imshow(divided)
...
and_image = and_images(image, image_new)
...
or_image = or_images(image, image_new)
plt.imshow(or_image)
...
xor_image = xor_images(image, image_new)
...
gray_level_bg = gray_level_back(image, 80, 200, highlight_value = 255)
plt.imshow(gray_level_bg)
...
gray_level_nobg = gray_level_withoutback(image, 80, 200, highlight_value = 255)
plt.imshow(gray_level_nobg)
...
result=contrast_stretching_gray(gray_image)
plt.imshow(result, cmap="gray")

contrast_stretched_rgb = contrast_stretching_rgb(image)
...
logged = log_transform(image, 20)
plt.imshow(logged)

hist, bins = np.histogram(logged.flatten(), bins=256, range=(0, 255))
...
powered = power_law(image, 7)
plt.imshow(powered)

hist, bins = np.histogram(powered.flatten(), bins=256, range=(0, 255))
plt.stairs(hist,bins)
...
result = histogram_equalization_gray(gray_image)
plt.imshow(result, cmap="gray")
...
def histogram_stretching(image, l, h):
    normalized_image = (image - image.min()) / (image.max() - image.min())
    stretched_image = ((h - l) * normalized_image) + l
...
strech_img=hist_streching(gray_image,0,255)
...
sns.histplot(gray_image.flatten(),bins=256)

sns.histplot(strech_img.flatten(),bins=256)

result1 = histogram_stretching(image,0,255)
plt.imshow(result1, cmap="gray")

hist, bins = np.histogram(gray_image.flatten(), bins=256, range=(0, 255))
plt.stairs(hist,bins)

hist, bins = np.histogram(result1.flatten(), bins=256, range=(0, 255))
plt.stairs(hist,bins)
...
new= add_gaussian_noise_gray(gray_image, mean=0, std=25)
...
new1=add_salt_and_pepper_noise(image, salt_prob=0.02, pepper_prob=0.02)
...
median_gray = median_filter_gray(new, 15)
plt.imshow(median_gray, cmap="gray")

median_gray = median_filter_gray(gray_image, 15)
plt.imshow(median_gray, cmap="gray")

median_rgb = median_filter_rgb(image, 15)
...
mean_gray = mean_filter_gray(gray_image, 15)
plt.imshow(mean_gray, cmap="gray")

mean_rgb = mean_filter_rgb(image, 15)
plt.imshow(mean_rgb)
...
result = low_pass_gray(gray_image, kernel)
plt.imshow(result, cmap='gray')
...
result = low_pass_rgb(image, kernel)
plt.imshow(result)
...
result = high_pass_gray(gray_image, kernel)
plt.imshow(result, cmap='gray')
...
result = high_pass_rgb(image, kernel)
plt.imshow(result)
...
result = high_boost_gray(gray_image, kernel)
plt.imshow(result, cmap='gray')
...
result = high_boost_rgb(image, kernel)
plt.imshow(result)
...
result = erode_image_gray(gray_image, se)
plt.imshow(result, cmap='gray')
...
result = erode_image_rgb(image, se)
plt.imshow(result)
...
result = dilate_image_gray(gray_image, se)
plt.imshow(result, cmap='gray')
...
result = dilate_image_rgb(image, se)
plt.imshow(result)
...
opening_image_gray = opening_gray(gray_image, se)
...
opening_image_rgb = opening_rgb(image, se)
...
closing_image_gray = closing_gray(gray_image, se)
...
closing_image_rgb = closing_rgb(image, se)
...
hit_and_miss_image = hit_and_miss_gray(gray_image, se1, se2)
...
hit_and_miss_image = hit_and_miss_rgb(image, se1, se2)
...
lpf_ideal = ideal_lowpass(gray_image.shape, 30)
filtered_image_ideal_lpf = apply_filter(gray_image, lpf_ideal)
...
lpf_ideal = ideal_highpass(gray_image.shape, 30)
filtered_image_ideal_hpf = apply_filter(gray_image, lpf_ideal)
...
lpf_ideal = gaussian_lowpass(gray_image.shape, 10)
filtered_image_ideal_lpf = apply_filter(gray_image, lpf_ideal)
plt.imshow(filtered_image_ideal_lpf, cmap="gray")
...
lpf_ideal = gaussian_highpass(gray_image.shape, 10)
filtered_image_ideal_hpf = apply_filter(gray_image, lpf_ideal)
plt.imshow(filtered_image_ideal_hpf, cmap="gray")
...
butter = butterworth_lowpass_filter(gray_image.shape, 10, 2)
butter_image = apply_filter(gray_image, butter)
...
seed_point = (100, 100)
threshold_value = 10
grown_region = region_growing(gray_image, seed_point, threshold_value)
